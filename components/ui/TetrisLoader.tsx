...
const TETRIS_PIECES = [
  // I-piece
  { shape: [[1, 1, 1, 1]], color: 'bg-black' },
  // O-piece
  { shape: [[1, 1], [1, 1]], color: 'bg-black' },
  // T-piece
  { shape: [[0, 1, 0], [1, 1, 1]], color: 'bg-black' },
  // L-piece
  { shape: [[1, 0], [1, 0], [1, 1]], color: 'bg-black' },
  // S-piece
  { shape: [[0, 1, 1], [1, 1, 0]], color: 'bg-black' },
  // Z-piece
  { shape: [[1, 1, 0], [0, 1, 1]], color: 'bg-black' },
  // J-piece
  { shape: [[0, 1], [0, 1], [1, 1]], color: 'bg-black' },
]
...
export default function TetrisLoading({ 
      ...
      if (linesToClear.length > 0) {
        ...
        const newGrid = prevGrid.map((row, rowIndex) => {
          if (linesToClear.includes(rowIndex)) {
            return row.map(cell => ({ ...cell, color: 'bg-black animate-pulse opacity-50' }))
          }
          return row
        })
      ...
      <div key={rowIndex} className="flex">
        {row.map((cell, colIndex) => (
          <div
            ...
            className={`${config.cellSize} border border-gray-300 transition-all duration-100 ${
              cell.filled 
  ...
  return (
    <div>
      <div className="mb-6">
        <div className={`border-2 border-gray-800 bg-white ${config.padding} transition-colors`}>
          {renderGrid()}
        </div>
      </div>
      ...
      {showLoadingText && (
        <div className="text-center">
          <p className="text-black font-medium transition-colors">{loadingText}</p>
        </div>
      )}
    </div>
  )
